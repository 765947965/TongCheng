...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private boolean isWithPadding;

	private boolean mIsBeingDragged = false;
	private State mState = State.RESET;
	public Mode mMode = Mode.getDefault();
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			if (mIsBeingDragged) {
				...
				if (overScrollListener != null) {
					overScrollListener.actionMove(mLastMotionY);
				} else {
					pullEvent();
				}
				return true;
		...
		case MotionEvent.ACTION_UP: {
			if (mIsBeingDragged) {
				...
				if (overScrollListener != null) {
					overScrollListener.actionUp(mLastMotionY);	
					callRefreshListener();
				}
	...
	public void showRefresh(Mode mode) {
		mRefreshType = 1;
		mCurrentMode = mode;
		onRefreshing(true, mode);
		mState = State.REFRESHING;
	}
	...
	final void setState(State state, final boolean... params) {
		...
		case RESET:
			onReset();
			if (isWithPadding) {
				getRefreshableView().setPadding(0, (int) (38 * mDisplayMetrics.density), 0, 0);
				getRefreshableView().setClickable(false);
			} else {
				getRefreshableView().setPadding(0, 0, 0, 0);
			}
			break;
	 ...
	 *            - Whether the UI should scroll for this event.
	 */
	protected void onRefreshing(final boolean doScroll, final Mode mode) {
		...
		if (mMode.showHeaderLoadingLayout()) {
			getRefreshableView().setPadding(0, 0, 0, 0);
			mHeaderLayout.setmRefreshType(mRefreshType);
			mHeaderLayout.refreshing();
		}
	...
	public void setWithPadding(boolean isWithPadding) {
		this.isWithPadding = isWithPadding;
	}
	...
	public interface OverScrollListener {
		public void actionMove(float scrollY);

		public void actionUp(float scrollY);
	}
